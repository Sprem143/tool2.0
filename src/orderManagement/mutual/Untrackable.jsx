...
export default function Untrackable({ state }) {
    ...
    useEffect(() => {
        state?.account && untrackable(state.account)
    }, [])
    ...
    async function submittrackingid(id) {
        if (!id) {
        ...
        res = await res.json();
        if (res.status) {
            setAlrtmsg('Tracking id updated')
            handleShowAlert()
                    ...
                    <div className="datacontainer" style={{ transition: '2s' }}>
                        <div className="container-fluid searchboxcontanier">
                            <div className="row d-flex align-items-center">
                                <div className="col-md-4 col-sm-6 tableheading"><img src="/static/untrackable.png" alt="" /> <TypeAnimation sequence={[`Untrackable -  ${result.length}`, 4000,]} wrapper="span" speed={20} />
                                </div>
                                        ...
                                        >
                                            ...
                                            <td className="w-25" style={{ placeItems: 'center', alignContent: 'center' }}>
                                                <div className="searchbox d-flex">
                                                    <input type="text" onChange={(e) => setTrackingid(e.target.value)} placeholder='Tracking id' />
                                                    <button onClick={() => submittrackingid(d._id)} className='border border-secondary ms-2 p-2 text-white'>
                                                        Submit
                                                    </button>
                                                </div>
                                            </td>
