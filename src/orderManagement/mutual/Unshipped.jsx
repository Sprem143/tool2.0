...
export default function Unshipped({ state }) {
    ...
    async function changestatus(id) {
        ...
        res = await res.json();
        if (res.status) {
            setAlrtmsg('Status of order updated')
            handleShowAlert()
            let newResult = result.filter((r) => r['ASINs'] !== res.asin);
            setResult(newResult)
        }
    }
    ...
    function setStatusorder(s, id){
        setBtnvalue((prev)=>({...prev, [id]:s}))
        setStatus(s)
    }
    return (
        <>
                ...
                <div className="datacontainer" style={{ transition: '2s' }}>
                    <div className="container-fluid searchboxcontanier">
                        <div className="row d-flex align-items-center">
                            <div className="col-md-4 col-sm-6 tableheading"><img src="/static/unshipped.png" alt="" /> <TypeAnimation sequence={[`Unshipped Orders -  ${result.length}`, 4000,]} wrapper="span" speed={20} />
                            </div>
                            ...
                            <tbody>
                                {
                                    result.map((d, i) => (
                                        ...
                                        >
                                                        ...
                                                        <Accordion.Body>
                                                            <div className="container-fluid">
                                                                <div className="row">
                                                                    ...
                                                                    <div className="col-md-3 col-sm-6 text-danger">
                                                                        Last Date - <b>{d['last date']}</b>
                                                                    </div>
                                                                </div>
                                                            </div>
                                                        </Accordion.Body>
                                            ...
                                            {state.role=='prep' &&
                                            <td className="w-25" style={{ placeItems: 'center', alignContent: 'center' }}>
                                                <div className="searchbox d-flex">
                                                <Dropdown as={ButtonGroup} key={d._id}>
  <Button variant="success pt-2 pb-2">
    {btnvalue[d._id] || 'Unshipped'}
  </Button>
  <Dropdown.Toggle split variant="success" id={`dropdown-${d._id}`} />
  <Dropdown.Menu>
    <Dropdown.Item onClick={() => setStatusorder("shipped", d._id)}>
      Shipped
    </Dropdown.Item>
    <Dropdown.Item onClick={() => setStatusorder("return", d._id)}>
      Return
    </Dropdown.Item>
  </Dropdown.Menu>
</Dropdown>
                                                    <button onClick={() => changestatus(d._id)} className='border border-secondary ms-2 p-2 text-white'>
                                                        Submit
                                                    </button>
                                                </div>
                                            </td>}

                                            {state.role == 'operator' && <td style={{ maxWidth: '100px' }}>
                ...
                : <div className="w-100 d-flex justify-content-end align-items-end">
                    <Button className="btn primary" onClick={showtable}>Need to ship order</Button>
                </div>}
        </>
    )
}